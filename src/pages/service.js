...
const ServicesPage = ({ data, location }) => {
  ...
  if (services.length === 0) {
    return (
      <Layout location={location}>
        <p>No service posts found. Add markdown posts to "content/service" (or the directory you specified for the "gatsby-source-filesystem" plugin in gatsby-config.js).</p>
      </Layout>
    );
  }
  ...
  return (
    <Layout location={location}>
      <Seo title="Services" />
      <h1 className={styles.serviceMenu}>MENU</h1>
        ...
        {services.map(service => {
          ...
          return (
            <div key={service.fields.slug} className={styles.serviceItem}>
              {image && (
                <GatsbyImage image={image} alt={service.frontmatter.title} className={styles.serviceImage} />
              )}
